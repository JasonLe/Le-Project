...
public class RedisUtil {
    ...
    private static final Jackson2JsonRedisSerializer<Object> JSON_SERIALIZER = getJsonSerializer();
    ...
    public void setExpire(final String key, final Object value, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            byte[] values = JSON_SERIALIZER.serialize(value);
    ...
    public void setExpire(final String[] keys, final Object[] values, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            for (int i = 0; i < keys.length; i++) {
                ...
                byte[] bValues = JSON_SERIALIZER.serialize(values[i]);
    ...
    public Object get(final String key) {
        Object resultStr = redisTemplate.execute((RedisCallback<Object>) connection -> {
            ...
            return JSON_SERIALIZER.deserialize(values);
        });
        log.debug("[redisTemplate redis]取出 缓存  url:{} ", key);
        return resultStr;
    }
        ...
        return redisTemplate.execute((RedisCallback<Map<String, Object>>) connection -> {
            ...
            if (CollectionUtils.isNotEmpty(keys)) {
                for (String key : keys) {
                    ...
                    Object value = JSON_SERIALIZER.deserialize(bValues);
                    maps.put(key, value);
                }
            }
            return maps;
